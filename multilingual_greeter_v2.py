...
def print_mode_options(mode_options: Dict[int,str]) -> None:
    ...
    :param mode_options: A dictionary
    choose the mode between admin and user
    """
    print("Please choose a mode: ")
    for num in mode_options:
        print(f"{num}: {mode_options.get(num)}")
    #pass

def add_additional_language(lang_options: Dict[int,str], lang_choice:int, language:str) -> None:
    """
    Under admin mode, Add support for additional languages.

    :param lang_options: the dictionary as choose a language

    :param lang_chioce: add a new chioce into dictionary

    :param language: add a language chioce into dictionary
    """
    lang_options.update(lang_choice,language)


def update_greetings(greeting_options: Dict[int,str], lang_choice:int, lang:str) -> None:
    """
    Update greetings for existing languages.

    """
    greeting_options.update(lang_choice,lang)
